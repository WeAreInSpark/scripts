...
foreach($event in $events){
    ...
    if($event.EventType -eq "Terminate" -and $instance_name -in $event.Resources){
        ...
        [void](Get-CimInstance -ClassName Win32_Product -Filter "Name='Microsoft Integration Runtime Preview' or Name='Microsoft Integration Runtime'" -ComputerName $env:COMPUTERNAME).Uninstall()
